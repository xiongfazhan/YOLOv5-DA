...
模型和数据集会自动从最新的YOLOv5发布中下载。
模型：https://github.com/ultralytics/yolov5/tree/master/models
数据集：https://github.com/positive666/yolov5/tree/master/data
教程：https://github.com/ultralytics/yolov5/wiki/Train-Custom-Data

用法：
    $ python path/to/train.py --data coco128.yaml --weights yolov5s.pt --img 640  # 从预训练模型开始训练（推荐）
    如果使用Swinv2,请添加 -swin_float
"""
...
LOCAL_RANK = int(os.getenv('LOCAL_RANK', -1))  # 获取环境变量中的LOCAL_RANK
RANK = int(os.getenv('RANK', -1))  # 获取环境变量中的RANK
WORLD_SIZE = int(os.getenv('WORLD_SIZE', 1))  # 获取环境变量中的WORLD_SIZE


def train(hyp, opt, device, callbacks):  # hyp是path/to/hyp.yaml或hyp字典
    ...
    callbacks.run('on_pretrain_routine_start')  # 运行回调函数

    compute_loss_ota=None  # 初始化compute_loss_ota为None
    # 目录
    ...
    opt.hyp = hyp.copy()  # 保存超参数到opt.hyp中，以便保存到检查点中
    ...
    if RANK in {-1, 0}:
        ...
        if resume:  # 如果从远程artifact恢复运行
            weights, epochs, hyp, batch_size = opt.weights, opt.epochs, opt.hyp, opt.batch_size

    # 配置
    plots = not evolve and not opt.noplots  # 是否创建图表
    ...
    assert len(names) == nc, f'{len(names)} names found for nc={nc} dataset in {data}'  # 检查
    ...
    if pretrained:
        with torch_distributed_zero_first(LOCAL_RANK):
            weights = attempt_download(weights)  # 如果本地没有找到权重文件，则下载
        ckpt = torch.load(weights, map_location='cpu')  # 将检查点加载到CPU上，以避免CUDA内存泄漏
        ...
        csd = ckpt['model'].float().state_dict()  # 将检查点state_dict转换为FP32
        csd = intersect_dicts(csd, model.state_dict(), exclude=exclude)  # 取交集
        model.load_state_dict(csd, strict=False)  # 加载
        LOGGER.info(f'Transferred {len(csd)}/{len(model.state_dict())} items from {weights}')  # 报告
    else:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--data', type=str, default=ROOT / 'data/daolu.yaml', help='dataset.yaml path')
